"""Session Memory Provider strategy implementation."""
...
class SessionMemoryProvider(IMemoryProvider):
    """Storage strategy for active session metadata, execution state, and user context."""

    def __init__(self) -> None:
        self._logger = StructuredLogger("SessionMemoryProvider")
    ...
    async def store(self, item: MemoryItem) -> str:
        ...
        self._logger.debug(f"Stored session memory '{item.memory_id}'")
    ...
    async def query(self, memory_query: MemoryQuery) -> List[MemoryItem]:
        ...
        for item in self._items.values():
            if memory_query.session_id and item.metadata.get("session_id") != memory_query.session_id:
                continue
